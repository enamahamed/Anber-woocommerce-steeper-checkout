...
function save_custom_return_address_fields($order, $data) {
    ...
    if (!empty($_POST['rtn_address_street'])) {
        $order->update_meta_data('_rtn_address_street', sanitize_text_field($_POST['rtn_address_street']));
    }
...
function display_return_address_fields_in_admin($order) {
    ...
    if ($order->get_meta('_rtn_address_street')) {
        echo '<p><strong>' . __('Street') . ':</strong> ' . esc_html($order->get_meta('_rtn_address_street')) . '</p>';
    }
...
//        echo '<h3>' . __('Return shipping Address Details', 'woocommerce') . '</h3>';
//        if (!empty($return_address_line)) {
//            echo '<p><strong>' . __('Address Line') . ':</strong> ' . esc_html($return_address_line) . '</p>';
//        }
//         if (!empty($rtn_address_street)) {
//            echo '<p><strong>' . __('City') . ':</strong> ' . esc_html($rtn_address_street) . '</p>';
//        }
//        if (!empty($return_address_city)) {
//            echo '<p><strong>' . __('City') . ':</strong> ' . esc_html($return_address_city) . '</p>';
//        }
//        if (!empty($return_address_post)) {
//            echo '<p><strong>' . __('Postcode') . ':</strong> ' . esc_html($return_address_post) . '</p>';
//        }
//        if (!empty($return_address_country)) {
//            echo '<p><strong>' . __('Country') . ':</strong> ' . esc_html($return_address_country) . '</p>';
//        }
//    }
//}
...
function display_return_address_on_my_account($order) {
    $return_address_line = $order->get_meta('_return_address_line');
    $rtn_address_street = $order->get_meta('_$rtn_address_street');
    $return_address_city = $order->get_meta('_return_address_city');
    $return_address_post = $order->get_meta('_return_address_post');
    $return_address_country = $order->get_meta('_return_address_country');

    if (!empty($return_address_line) || !empty($rtn_address_street) || !empty($return_address_city) || !empty($return_address_post) || !empty($return_address_country)) {
        echo '<h3>' . __('Return shipping Address Details', 'woocommerce') . '</h3>';
        if (!empty($return_address_line)) {
            echo '<p><strong>' . __('Address Line') . ':</strong> ' . esc_html($return_address_line) . '</p>';
        }
        if (!empty($rtn_address_street)) {
            echo '<p><strong>' . __('Street') . ':</strong> ' . esc_html($rtn_address_street) . '</p>';
        }
        if (!empty($return_address_city)) {
            echo '<p><strong>' . __('City') . ':</strong> ' . esc_html($return_address_city) . '</p>';
        }
...
function add_returning_documents_fee() {
    // Check if the returning documents option is set in the POST data
    if (!empty($_POST['returning_documents_option'])) {
        // Clean and split the input
        $option = wc_clean($_POST['returning_documents_option']);
        list($title, $price) = explode('|', $option);

        // Validate the price
        $price = floatval($price);

        // Add the fee with the title and price
        if ($price > 0) {
            WC()->cart->add_fee(__('Returning Documents: ' . esc_html($title), 'your-text-domain'), $price);
        }
    }
}
